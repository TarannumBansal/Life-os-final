...
import { ScheduleDaySheet } from "@/src/components/ScheduleDaySheet";
          ...
          <Container>
            ...
            <View style={styles.grid}>
              {cells.map((cell, i) => {
                ...
                const hasLogs = scheduleBlocksForDate(settings.scheduleLogs, cell.iso).length > 0;
                return (
                  <Pressable key={i} onPress={() => setSheetDate(cell.iso)} style={[styles.cell, { borderColor: c.border }]} testID={`cal-cell-${cell.iso}`}>
                    ...
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 3 }}>
                      ...
                      {hasLogs ? <View style={{ width: 5, height: 5, borderRadius: 3, backgroundColor: c.warning }} /> : null}
                    </View>
                  </Pressable>
                );
              })}
            ...
            <View style={{ marginTop: s.lg }}>
              ...
              <Text style={{ color: c.textDim, fontSize: 12, lineHeight: 20 }}>
                Tap any date to add or edit a real-life commitment. · DO1–5 = Day Order · STEP = Saturday · Rev = Sunday review · Exam/Hol · Flex · W# = week · orange dot = commitment
              </Text>
            </View>
          </Container>
        </ScrollView>
      </SafeAreaView>
      {sheetDate ? <ScheduleDaySheet dateISO={sheetDate} visible={!!sheetDate} onClose={() => setSheetDate(null)} /> : null}
    </View>
  );
}
