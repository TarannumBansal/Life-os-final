  ...
  return (
    <View style={[styles.card, { backgroundColor: c.surface2, borderColor: c.border }]} testID={`block-${block.id}`}>
      <View style={styles.header}>
        ...
        <Text style={[styles.progress, { color: doneCount === totalTasks && totalTasks > 0 ? c.success : c.textDim, fontFamily: t.monoFont }]}>{doneCount}/{totalTasks}{doneCount === totalTasks && totalTasks > 0 ? " ✓" : ""}</Text>
      </View>
      ...
      {block.tasks.map((task) => {
        ...
        return (
          ...
          >
            <View style={[styles.check, { borderColor: done ? c.success : c.borderStrong, backgroundColor: done ? c.success : "transparent" }]}>
...
const styles = StyleSheet.create({
  ...
  task: { flexDirection: "row", gap: s.md, paddingVertical: s.md },
