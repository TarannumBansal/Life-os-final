...
export function useSync(intervalMs = 20000) {
  ...
  const runSync = useCallback(async () => {
    ...
    try {
      ...
      const settings = await storage.getItem<LifeOSSettings | null>(SETTINGS_KEY, null as any);
      ...
      // Apply reconciled remote changes back to local storage (screens pick up on next mount).
      const remoteOnly = outcome.merged.filter((r) => r.updatedAt > cursor);
      if (remoteOnly.length) {
        const merged = applyRecordsToSettings(settings, remoteOnly);
        await storage.setItem(SETTINGS_KEY, merged as any);
      }
